...
macro_rules! try_partial (
    ($try:expr) => {
        match $try {
            SaveResult::Full(full) => return SaveResult::Full(full.into()),
            SaveResult::Partial(partial, reason) => (partial, reason),
            SaveResult::Error(e) => return SaveResult::Error(e),
        }
    }
);
...
impl<'m, M: 'm> SaveBuilder<&'m mut MultipartData<M>> where MultipartData<M>: BufRead {
    ...
    pub fn with_path<P: Into<PathBuf>>(&mut self, path: P) -> FieldSaveResult {
        let bytes = if self.text_policy != TextPolicy::Ignore {
            let (text, reason) = try_partial!(self.save_text());

            match reason {
                PartialReason::SizeLimit if !self.cmp_size_limit(text.len()) => text.into_bytes(),
                PartialReason::Utf8Error(_) if self.text_policy != TextPolicy::Force => text.into_bytes(),
                other => return SaveResult::Partial(text.into(), other),
            }
        } else {
            Vec::new()
        };
        ...
        match reason {
            PartialReason::SizeLimit if !self.cmp_size_limit(bytes.len()) => (),
            other => return SaveResult::Partial(bytes.into(), other)
        }
    ...
    fn save_mem(&mut self, mut bytes: Vec<u8>) -> SaveResult<Vec<u8>, Vec<u8>> {
        ...
        match self.read_mem(|buf| { bytes.extend_from_slice(buf); SaveResult::Full(buf.len()) }, pre_read) {
            SaveResult::Full(_) => SaveResult::Full(bytes),
            SaveResult::Partial(_, reason) => SaveResult::Partial(bytes, reason),
            SaveResult::Error(e) => if !bytes.is_empty() { SaveResult::Partial(bytes, e.into()) }
            else { SaveResult::Error(e) }
        }
    ...
    fn save_text(&mut self) -> SaveResult<String, String> {
        ...
        let res = self.read_mem(|buf| {
            match str::from_utf8(buf) {
                Ok(s) => { string.push_str(s); SaveResult::Full(buf.len()) },
                // buffer should always be bigger
                Err(e) => if buf.len() < 4 { return SaveResult::Partial(0, e.into())} else {
                    string.push_str(str::from_utf8(&buf[..e.valid_up_to()]).unwrap());
                    SaveResult::Full(e.valid_up_to())
                }
            }
        }, 0);

        match res {
            SaveResult::Full(_) => SaveResult::Full(string),
            SaveResult::Partial(_, reason) => SaveResult::Partial(string, reason),
...
impl<M: ReadEntry> EntriesSaveResult<M> {
    ...
    pub fn into_entries(self) -> Option<Entries> {
        match self {
            SaveResult::Full(entries) | SaveResult::Partial(PartialEntries { entries, .. }, _) => Some(entries),
            SaveResult::Error(_) => None,
        }
    }
}
...
impl<S, P> SaveResult<S, P> where P: Into<S> {
    ...
    pub fn map<T, Map>(self, map: Map) -> SaveResult<T, T> where Map: FnOnce(S) -> T {
        match self {
            SaveResult::Full(full) => SaveResult::Full(map(full)),
            SaveResult::Partial(partial, reason) => SaveResult::Partial(map(partial.into()), reason),
            SaveResult::Error(e) => SaveResult::Error(e),
        }
    }
    ...
    pub fn into_opt_both(self) -> (Option<S>, Option<io::Error>) {
        match self {
            SaveResult::Full(full)  => (Some(full), None),
            SaveResult::Partial(partial, PartialReason::IoError(e)) => (Some(partial.into()), Some(e)),
            SaveResult::Partial(partial, _) => (Some(partial.into()), None),
            SaveResult::Error(error) => (None, Some(error)),
        }
    }
    ...
    pub fn into_result(self) -> io::Result<S> {
        match self {
            ...
            SaveResult::Partial(partial, _) => Ok(partial.into()),
    ...
    pub fn into_result_strict(self) -> io::Result<S> {
        match self {
            SaveResult::Full(entries) => Ok(entries),
            SaveResult::Partial(_, PartialReason::IoError(e)) | SaveResult::Error(e) => Err(e),
            SaveResult::Partial(partial, _) => Ok(partial.into()),
        }
    }
}
...
fn try_copy_limited<R: BufRead, Wb: FnMut(&[u8]) -> SaveResult<usize, usize>>(src: R, mut with_buf: Wb, limit: u64) -> SaveResult<u64, u64> {
    ...
    try_read_buf(src, |buf| {
        ...
        if new_copied > limit { return SaveResult::Partial(0, PartialReason::SizeLimit) }
        copied = new_copied;
...
fn try_read_buf<R: BufRead, Wb: FnMut(&[u8]) -> SaveResult<usize, usize>>(mut src: R, mut with_buf: Wb) -> SaveResult<u64, u64> {
    ...
    macro_rules! try_here (
        ($try:expr) => (
            match $try {
                ...
                Err(e) => return if total_copied == 0 { SaveResult::Error(e) }
                                 else { SaveResult::Partial(total_copied, e.into()) },
            }
        )
    );
    ...
    loop {
        ...
        match res {
            SaveResult::Full(copied) => { src.consume(copied); total_copied += copied as u64; }
